...
class Classs extends React.Component {
  ...
  numOfPart() { //returns the number of participants in this class.. if there is not, returns that there is not yet
    ...
    if (this.state.thisClass.numOfCurrPartici <= 0) {
      ...
      howManyPart =
        "משתתפים כרגע " + this.state.thisClass.numOfCurrPartici + " אנשים";
    }
    return howManyPart;
  }
  ...
  ifAlreadyParti() { //returns Cancel button if the user is already in, and Join if else
    let button = (
      <Button
        ...
        variant="contained"
        color="primary"
        onClick={() => this.whenJoinClicked()}
      >
        Join
      </Button>
    );
    if (this.state.thisClass.partiList === undefined) return null;
    this.state.thisClass.partiList.forEach(participant => {
      if (
        firebase.auth().currentUser !== null &&
        firebase.auth().currentUser.uid === participant.id
      )
        button = (
          <Button
            className="button1"
            variant="contained"
            color="primary"
            onClick={() => this.whenCancelClicked()}
          >
            ביטול רישום
          </Button>
        );
  ...
  render() {
    ...
    if (!this.state.isLogin && this.state.isJoinClicked) sendToLogin = true;
    if (this.state.thisClass.numOfCurrPartici === this.state.thisClass.maxParti)
      this.setState.ifClassFull = true;
    //the class is full
      ...
      <div className="mainDiv">
        {sendToLogin ? (
          <Redirect
            to={{
              ...
              state: {
                location: location1,
                title: "על מנת להרשם לקורס צריך להתחבר"
              }
            }}
          />
        ) : null}
        ...
        <div className="leftSide">
          <CardImg
            className="classImg"
            variant="top"
            src={this.state.thisClass.img}
          />
          <div>
            {this.setState.ifClassFull
              ? classFullMessage
              : this.ifAlreadyParti()}
          </div>
        </div>
          ...
          <Card style={{ width: "30rem" }}>
            <CardBody>
              <CardTitle className="title">
                {this.state.thisClass.name}
              </CardTitle>
              <ListGroup className="list-group-flush">
                <p>{this.state.thisClass.description}</p>
                <ListGroupItem className="location">
                  {" "}
                  {this.state.thisClass.location}
                </ListGroupItem>
                <ListGroupItem className="numOfCurrPartici">
                  {this.numOfPart()}
                </ListGroupItem>
              </ListGroup>
            </CardBody>
